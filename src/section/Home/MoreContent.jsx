...
export default function MoreContent() {
  ...
  const articles = works.filter((work) => work.type === "Article").slice(0, 3); // Update this line
    ...
    <>
      <div className="py-28 px-12 max-sm:px-4 max-sm:py-10 max-sm:pt-20">
        ...
        <div className="flex justify-center items-center 2xl:gap-10 max-md:gap-0   flex-wrap max-md:flex-col w-full">
          <div className="flex justify-center items-start 2xl:gap-10 max-md:gap-0   flex-wrap max-md:flex-col ">
            {articles.map((art) => (
              <div
                key={art.id}
          ...
          <Link
            to="/Work/Non-Fiction"
            className="h-full flex justify-center items-center px-2 py-1 rounded-full border-2 border-black"
          >
            <span className="text-xl max-sm:text-base">More</span>
            <img
              src={ArrowRightBlack}
